...
import { fetchEspacos } from '../services/application.Services';
...
class EspacosControl extends React.Component {
    ...
    buscarEspaco = async () => {
        const { condominio_id } = this.props;
        try {
            const espacos = await fetchEspacos(condominio_id);
    ...
    deletarEspaco = (id) => {
        fetch(`${baseURL}/espaco/` + id, { method: 'DELETE' })
            .then(resposta => {
                if (resposta.ok) {
                    this.buscarEspaco();
                }
    ...
    carregarEspaco = (id) => {
        fetch(`${baseURL}/espaco/` + id, { method: 'GET' })
    ...
    atualizarEspaco = (espaco) => {
        fetch(`${baseURL}/espaco/` + espaco.id, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(espaco)
        })
